...
void Player::handle_events(float dt)
{
	...
	int mouse_x, mouse_y;
	SDL_GetRelativeMouseState(&mouse_x_old, &mouse_y_old);
    while(SDL_PollEvent(&event))
    {
        ...
        if (menu->current == None) SDL_SetRelativeMouseMode(SDL_TRUE);
        if ((event.type == SDL_MOUSEMOTION) && (menu->current == None)){
			SDL_GetRelativeMouseState(&mouse_x, &mouse_y);
			if (mouse_x > 0){
				//angle += (mouse_x - mouse_x_old) * 1 * dt;
				//mouse_x_old += mouse_x - mouse_x_old;
				//mouse_x_old = 1280 / 2;
				angle += 0.4*mouse_x * dt;
			}
			if (mouse_x < 0){
				//angle -= (mouse_x_old - mouse_x) * 1 * dt;
				//mouse_x_old -= mouse_x_old - mouse_x;
				//mouse_x_old = 1280 / 2;
				angle += 0.4*mouse_x * dt;
			}
			//angle += turn;
...
void Player::update_key(SDL_Keycode key, bool state)
{
    ...
    else if (key == SDL_MOUSEBUTTONDOWN && menu->current != Pause) pressed_keys[7] = state;
}
                ...
                if(sprites.at(i).type == Enemy2)
                {
                   float sqr_dist = pow(ray_x - sprites.at(i).x, 2) + pow(ray_y - sprites.at(i).y, 2);
                    if(sqr_dist < 0.016)
                    {
                        ...
                        map->delete_sprite(i);
                        hit_wall = true;
                        map->add_temp_sprite(11, sprites.at(i).x, sprites.at(i).y, 400);
                        map->sort_sprites(x, y);
                ...
                if(sprites.at(i).type == Enemy3)
                {
                   float sqr_dist = pow(ray_x - sprites.at(i).x, 2) + pow(ray_y - sprites.at(i).y, 2);
                    if(sqr_dist < 0.016)
                    {
                        ...
                        map->delete_sprite(i);
                        hit_wall = true;
                        map->add_temp_sprite(17, sprites.at(i).x, sprites.at(i).y, 400);
                        map->sort_sprites(x, y);
