...
void Renderer::draw(uint fps)
{
            ...
            if(valid)
            {
                ...
                for(int j = 0; j < screen_h; j++)
                {
                    if(j < wall_top) //draw sky (top : 0,50,200 ; bottom : 150,200,200)
                        set_pixel(i, j, rgb_to_int(50 * (j / (float)screen_h * 2), 50 + 150 * (j / (float)screen_h * 2), 200));
                        //set_pixel(i, j, rgb_to_int(50 * (j / (float)screen_h * 2), 50 + 150 * (j / (float)screen_h * 2), 200));
    ...
    if(menu->current == None)
    {
        ...
        //FPS weapon
        int offset = screen_w / 2 - 192 - 5;
        if(player->display_flash)
        {
			sprites.erase(sprites.begin() + 0);
			draw_2d_sprite(3, offset, screen_h - 400, 400.0);
			draw_2d_sprite(12, offset, screen_h - 400, 400.0);
...
{   
    draw_text(50, 50, "Suspicious Demo v0.5", true, ttf_color_white);
...
{   
    ...
    draw_text(20, 200, "Move : W/S/A/D", false, ttf_color_white);
    draw_text(20, 270, "Camera : Left and right arrows or mouse", false, ttf_color_white);
    draw_text(20, 340, "Fire : Spacebar or Left Click", false, ttf_color_white);
