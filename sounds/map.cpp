	...
	for(ushort y = 0; y < h; y++)
	{
		for(ushort x = 0; x < w; x++)
		{
			...
			if(pixel == 0) //black 0,0,0 : grass
				...
				map[id] = '2';
			else if(pixel == 4934475) 
				map[id] = '5';
			else if(pixel == 6579300) //blue 0,0,255 : cobblestone
				map[id] = '4';
			...
			else if(pixel == 16711680) //red 255,0,0 : enemy
			{
				...
				sprites.at(index).size = 800 + ((id * 50) % 100);
				enemy_count++;
			}
			else if(pixel == 13121335) // enemy2
			{
				map[id] = ' ';
				...
				sprites.at(index).x = x + (std::rand() % 100) / 100.0;
				sprites.at(index).y = y + (std::rand() % 100) / 100.0;
				sprites.at(index).itex = std::rand() % 2 == 9 ? 9 : 10;
				sprites.at(index).type = Enemy2;
				sprites.at(index).size = 600 + ((id * 50) % 100);
				enemy_count++;
			}
			else if(pixel == 9840680) // enemy3
			{
				map[id] = ' ';
				...
				sprites.at(index).x = x + (std::rand() % 100) / 100.0;
				sprites.at(index).y = y + (std::rand() % 100) / 100.0;
				sprites.at(index).itex = std::rand() % 2 == 13 ? 13 : 14;
				sprites.at(index).type = Enemy3;
				sprites.at(index).size = 700 + ((id * 50) % 100);
				enemy_count++;
			}
			else if(pixel == 16727100) // enemy4
			{
				map[id] = ' ';
				...
				sprites.at(index).x = x + (std::rand() % 100) / 100.0;
				sprites.at(index).y = y + (std::rand() % 100) / 100.0;
				sprites.at(index).itex = std::rand() % 2 == 15 ? 15 : 16;
				sprites.at(index).type = Enemy4;
				sprites.at(index).size = 1000 + ((id * 50) % 100);
				enemy_count++;
			}
			...
			else if(pixel == 47615) //decoration
			{
				map[id] = ' ';
				unsigned int index = sprites.size();
				sprites.push_back(Sprite());
...
int Map::damage_player()
{
	...
	for(unsigned int i = 0; i < sprites.size(); i++)
	{
		if(sprites.at(i).type == Enemy && sprites.at(i).sqr_dist < 2)
			amount += damage;
		if(sprites.at(i).type == Enemy2 && sprites.at(i).sqr_dist < 2)
			amount += damage;
		if(sprites.at(i).type == Enemy3 && sprites.at(i).sqr_dist < 2)
			amount += damage;
		if(sprites.at(i).type == Enemy4 && sprites.at(i).sqr_dist < 2)
			amount += damage;
	}
	return amount;
}
...
void Map::animate_sprites()
{
	for(unsigned int i = 0; i < sprites.size(); i++)
	{
		...
		if(sprites.at(i).type == Enemy2)
			sprites.at(i).itex = sprites.at(i).itex == 9 ? 10 : 9;
		if(sprites.at(i).type == Enemy3)
			sprites.at(i).itex = sprites.at(i).itex == 13 ? 14 : 13;
		if(sprites.at(i).type == Enemy4)
			sprites.at(i).itex = sprites.at(i).itex == 15 ? 16 : 15;
	}
}
...
void Map::update_sprites(float player_x, float player_y, float dt)
{
	...
	for(unsigned int i = 0; i < sprites.size(); i++)
	{
		if((sprites.at(i).type == Enemy) || (sprites.at(i).type == Enemy2) || (sprites.at(i).type == Enemy3) || (sprites.at(i).type == Enemy4))
		{
